    ...
    if gamePlayAlready == True:
        ...
        for gameId in gameIdSet:
            ...
            for player in box.get_dict()["homeTeam"]["players"]:
                ...
                playerStatus["投籃命中率"] = (
                    str(int(player["statistics"]["fieldGoalsPercentage"] * 100)) + "%"
                )
                playerStatus["三分進球數"] = player["statistics"]["threePointersMade"]
                playerStatus["三分命中率"] = (
                    str(int(player["statistics"]["threePointersPercentage"] * 100))
                    + "%"
                )
                playerStatus["罰球進球數"] = player["statistics"]["freeThrowsMade"]
                playerStatus["罰球命中率"] = (
                    str(int(player["statistics"]["freeThrowsPercentage"] * 100)) + "%"
                )
                ...
                playerStatus["上場時間"] = player["statistics"]["minutesCalculated"][
                    2:4
                ]
        ...
        allGameStatics_DF = allGameStatics_DF.sort_values(

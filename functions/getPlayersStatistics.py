...
def getPlayersStatistics(sort_columns=["得分"]):
    ...
    if all_games_done and all_games_data:
        try:
    ...
    elif not all_games_done:
        return "部分比賽仍未開打 請稍候再試"

    elif not all_games_data:
        return "比賽尚未開打 請稍候再試"


def getPlayerData(player, team_data):
    return {
...
def getGameData(game_id_array):
    ...
    for game_id in game_id_array:
        game_data = boxscore.BoxScore(game_id).game
        game_data.append(game_data)
...
def getGameStatistics(game_data, sort_columns):
    ...
    for data in game_data:
        one_game_statics = []
        for player in data.get_dict()["homeTeam"]["players"]:
            player_data = getPlayerData(player, data.get_dict()["homeTeam"])
            one_game_statics.append(player_data)

        for player in data.get_dict()["awayTeam"]["players"]:
            player_data = getPlayerData(player, data.get_dict()["awayTeam"])
            one_game_statics.append(player_data)
